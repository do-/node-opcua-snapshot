...
const {nodeId} = Snapshot, browseName = o => ({ namespaceIndex: o.ns, name: o.name }), idName = o => ({browseName: browseName(o), nodeId: nodeId(o) })
...
export default (class extends EventEmitter {
    ...
    setValues (nodeOrId, values, dates, status) {
        ...
        for (let i = 0; i < dates.length; i++) setValueFromSource(node, values[i], status, dates[i])
