...
const NS0 = 'ns=0;i=', ns0id = (name, s, postfix = 'TypeIds') => {
    const k = name + postfix
    if (!(k in opcua)) return undefined
...
export default (class extends EventEmitter {
    ...
    async browse (nodeId) {
        ...
        try {
            const {statusCode, references} = await this.#session.browse (nodeId)
            if (statusCode.isGoodish()) return references
            this.emit('warning', statusCode);
            return [];
        }
        finally {
            this.emit ('finish')
...
// if (reference.browseName.name === 'A1') console.log (reference)
    ...
    toRootObject (arg = {}) {
        ...
        const { ns, id, name, type } = { ...DEFAULT_ROOT, ...arg }
